...
#define BI_ZREAD(filefunc,filestream,buf,size) ((*((filefunc).zread_file))((filefunc).opaque,filestream,buf,size))
#define BI_ZWRITE(filefunc,filestream,buf,size) ((*((filefunc).zwrite_file))((filefunc).opaque,filestream,buf,size))
#define BI_ZTELL(filefunc,filestream) ((*((filefunc).ztell_file))((filefunc).opaque,filestream))
#define BI_ZSEEK(filefunc,filestream,pos,mode) ((*((filefunc).zseek_file))((filefunc).opaque,filestream,pos,mode))
#define BI_ZCLOSE(filefunc,filestream) ((*((filefunc).zclose_file))((filefunc).opaque,filestream))
#define BI_ZERROR(filefunc,filestream) ((*((filefunc).zerror_file))((filefunc).opaque,filestream))

typedef voidp BI_UnzFile;
...
typedef struct _BI_UnzFileInfo_
{
...
extern BI_UnzFile BIUnzOpen(const char *path);
/*
...
extern int BIUnzClose(BI_UnzFile file);
/*
...
extern int BIUnzGoToFirstFile(BI_UnzFile file);
/*
 Set the current file of the zipfile to the first file.
 return kUnzOK if there is no problem
 */

extern int BIUnzGoToNextFile(BI_UnzFile file);
/*
...
extern int BIUnzGetCurrentFileInfo(BI_UnzFile file,
...
extern int BIUnzOpenCurrentFile(BI_UnzFile file);
/*
 Open for reading data the current file in the zipfile.
 If there is no error, the return value is kUnzOK.
 */

extern int BIUnzCloseCurrentFile(BI_UnzFile file);
/*
 Close the file in zip opened with unzOpenCurrentFile
 Return kUnzCrcError if all the file was read but the CRC is not good
 */

extern int BIUnzReadCurrentFile(BI_UnzFile file,voidp buf,unsigned len);
/*
