...
typedef struct _BI_UnzFileInfoInterl_
{
    uLong offset_curfile;/* relative offset of local header 4 bytes */
}BI_UnzFileInfoInterl;


/* BI_FileInZipReadInfo contain internal information about a file in zipfile,
...
typedef struct
{
    bi_zlib_filefunc_def z_filefunc;
    voidpf filestream;        /* io structore of the zipfile */
    BI_UnzGlobalInfo gi;       /* public global information */
    uLong byte_before_the_zipfile;/* byte before the zipfile, (>0 for sfx)*/
    ...
    BI_UnzFileInfo cur_file_info; /* public info about the current file in zip*/
    BI_UnzFileInfoInterl cur_file_info_internal; /* private info about it*/
    BI_FileInZipReadInfo* pfile_in_zip_read; /* structure about the current
...
{
    unsigned char c;
    int err = (int)BI_ZREAD(*pzlib_filefunc_def,filestream,&c,1);
    if (err==1)
    {
        *pi = (int)c;
        return kUnzOK;
    ...
    {
        if (BI_ZERROR(*pzlib_filefunc_def,filestream))
            return kUnzError;
        else
            return kUnzEof;
    }
...
local uLong bi_unzlocal_SearchCentralDir(const bi_zlib_filefunc_def* pzlib_filefunc_def, voidpf filestream)
{
    ...
    if (BI_ZSEEK(*pzlib_filefunc_def,filestream,0,KZLIB_FILEFUNC_SEEK_END) != 0)
        return 0;
    
    
    uSizeFile = BI_ZTELL(*pzlib_filefunc_def,filestream);
    ...
    while (uBackRead<uMaxBack)
    {
        ...
        if (BI_ZSEEK(*pzlib_filefunc_def,filestream,uReadPos,KZLIB_FILEFUNC_SEEK_SET)!=0)
            break;
        
        if (BI_ZREAD(*pzlib_filefunc_def,filestream,buf,uReadSize)!=uReadSize)
            break;
...
BI_UnzFile BIUnzOpen(const char * path)
{
    BI_Unz us;
    BI_Unz *s;
    ...
    if (BI_ZSEEK(us.z_filefunc, us.filestream,
    ...
    if (err!=kUnzOK)
    {
        BI_ZCLOSE(us.z_filefunc, us.filestream);
        return NULL;
    }
    ...
    s=(BI_Unz*)ALLOC(sizeof(BI_Unz));
    *s=us;
    BIUnzGoToFirstFile((BI_UnzFile)s);
    return (BI_UnzFile)s;
}
...
int BIUnzClose(BI_UnzFile file)
{
    BI_Unz* s;
    if (file==NULL)
    ...
    if (s->pfile_in_zip_read!=NULL)
        BIUnzCloseCurrentFile(file);
    
    BI_ZCLOSE(s->z_filefunc, s->filestream);
    TRYFREE(s);
    return kUnzOK;
}
 ...
 */
local int bi_unzlocal_GetCurrentFileInfoInternal (BI_UnzFile file,
                                                  BI_UnzFileInfo *pfile_info,
                                                  BI_UnzFileInfoInterl
                                                  *pfile_info_internal,
                                                  char *szFileName,
...
{
    BI_Unz* s;
    BI_UnzFileInfo file_info;
    BI_UnzFileInfoInterl file_info_internal;
    int err=kUnzOK;
    uLong uMagic;
    long lSeek=0;
    ...
    if (file==NULL)
        return kUnzParamerror;
    s=(BI_Unz*)file;
    if (BI_ZSEEK(s->z_filefunc, s->filestream,
    ...
    if ((err==kUnzOK) && (szFileName!=NULL))
    {
        ...
        {
            if (BI_ZREAD(s->z_filefunc, s->filestream,szFileName,uSizeRead)!=uSizeRead)
            {
                err=kUnzError;
            }
    ...
    if ((err==kUnzOK) && (extraField!=NULL))
    {
        ...
        if (lSeek!=0)
        {
            if (BI_ZSEEK(s->z_filefunc, s->filestream,lSeek,KZLIB_FILEFUNC_SEEK_CUR)==0)
                lSeek=0;
            else
                err=kUnzError;
        }
        if ((file_info.size_file_extra>0) && (extraFieldBufferSize>0))
            if (BI_ZREAD(s->z_filefunc, s->filestream,extraField,uSizeRead)!=uSizeRead)
                err=kUnzError;
    ...
    if ((err==kUnzOK) && (szComment!=NULL))
    {
        ...
        if ((file_info.size_file_comment>0) && (commentBufferSize>0))
            if (BI_ZREAD(s->z_filefunc, s->filestream,szComment,uSizeRead)!=uSizeRead)
                err=kUnzError;
...
/*
 ...
 return kUnzOK if there is no problem
 */
int BIUnzGoToFirstFile(BI_UnzFile file)
{
    int err=kUnzOK;
    BI_Unz* s;
    if (file==NULL)
 ...
 */
int  BIUnzGoToNextFile (BI_UnzFile file)
{
    BI_Unz* s;
    int err;
                                                   ...
                                                   uInt  *psize_local_extrafield)
{
    ...
    *psize_local_extrafield = 0;
    ...
    if (BI_ZSEEK(s->z_filefunc, s->filestream,s->cur_file_info_internal.offset_curfile +
...
int BIUnzOpenCurrentFile(BI_UnzFile file)
{
    int err=kUnzOK;
    uInt iSizeVar;
    BI_Unz* s;
    BI_FileInZipReadInfo* pfile_in_zip_read_info;
    uLong offset_local_extrafield;  /* offset of the local extra field */
    uInt  size_local_extrafield;    /* size of the local extra field */
    ...
    pfile_in_zip_read_info = (BI_FileInZipReadInfo*)
    ALLOC(sizeof(BI_FileInZipReadInfo));
 ...
 */
int  BIUnzReadCurrentFile(BI_UnzFile file,voidp buf,unsigned len)
{
    int err=kUnzOK;
    uInt iRead = 0;
    BI_Unz* s;
    ...
    while (pfile_in_zip_read_info->stream.avail_out>0)
    {
            ...
            (pfile_in_zip_read_info->rest_read_compressed>0))
        {
            ...
            if (uReadThis == 0)
                return kUnzEof;
            if (BI_ZSEEK(pfile_in_zip_read_info->z_filefunc,
                      pfile_in_zip_read_info->filestream,
                      pfile_in_zip_read_info->pos_in_zipfile +
                      pfile_in_zip_read_info->byte_before_the_zipfile,
                      KZLIB_FILEFUNC_SEEK_SET)!=0)
                return kUnzError;
            if (BI_ZREAD(pfile_in_zip_read_info->z_filefunc,
                      pfile_in_zip_read_info->filestream,
 ...
 */
int BIUnzCloseCurrentFile(BI_UnzFile file)
{
    int err=kUnzOK;
