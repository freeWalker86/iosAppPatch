...
@interface FWIvarInfo(){
	NSString * name;
	NSString * typeEncoding;
}
@end

@implementation FWIvarInfo
...
+ (FWIvarInfo *)ivarInfoWithName:(NSString *)aName type:(NSString *)aType{
	FWIvarInfo * obj = [[FWIvarInfo alloc] initWithName:aName type:aType];
#if !__has_feature(objc_arc)
...
#pragma mark ///////////////////////////////////////////////////
@implementation NSObject (FWIvarAccess)
...
- (NSArray *)getIvars {
	...
	for (unsigned int i = 0; i < count; i++) {
		...
		[infoObjects addObject:[FWIvarInfo ivarInfoWithName:name type:type]];
	}
